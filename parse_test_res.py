...
def parse_function(*metrics, directory="", args=None, end_signal=None):
    ...
    for subdir in subdirs:
        ...
        # fpath = osp.join(directory, subdir, "finetune", "log.txt")
...
def main(args, end_signal):
    ...
    middle_path = "shots_16/MM_CLS_OP/vit_b16_c4_ep50_imagenet21k_pretrain"
    if args.multi_exp:
        final_results = defaultdict(list)

        for directory in listdir_nohidden(args.directory, sort=True):
            directory = osp.join(args.directory, directory, middle_path)
