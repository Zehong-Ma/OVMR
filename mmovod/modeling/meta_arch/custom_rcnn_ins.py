...
class CustomRCNNINS(GeneralizedRCNN):
    '''
    Add image labels
    '''
    @configurable
    ...
    def inference(
        ...
        if do_postprocess:
            ...
            return CustomRCNNINS._postprocess(
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        if self.max_num_box>0 and self.training:
            ...
            # if len(gt_instances)> batch_size_per_card*self.max_num_box:
            #     gt_instances = random.sample(gt_instances, k=batch_size_per_card*self.max_num_box)
            # print("gt_ins_num: ", gt_nums)
        # max_labels_ = torch.max(torch.tensor([(gt_ins.gt_classes).max() for gt_ins in gt_instances]))
        # print(max_labels_)
