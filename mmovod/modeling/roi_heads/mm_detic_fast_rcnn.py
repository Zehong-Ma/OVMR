...
from .mm_zero_shot_classifier import MMZeroShotClassifier

__all__ = ["MMDeticFastRCNNOutputLayers"]


class MMDeticFastRCNNOutputLayers(FastRCNNOutputLayers):
    @configurable
    def __init__(
        self, 
    ...
    def from_config(cls, cfg, input_shape):
        ...
        if ret['use_zeroshot_cls']:
            ret['cls_score'] = None #MMZeroShotClassifier(cfg, input_shape)
        return ret
    ...
    def losses(self, predictions, proposals, \
        ...
        """
        scores, proposal_deltas, scores_list = predictions
        ...
        if self.use_sigmoid_ce:
            loss_cls_mm = self.sigmoid_cross_entropy_loss(scores_list[0], gt_classes)
            loss_cls_v = self.sigmoid_cross_entropy_loss(scores_list[1], gt_classes)
            loss_cls_t = self.sigmoid_cross_entropy_loss(scores_list[2], gt_classes)
            # loss_cls_fuse = self.sigmoid_cross_entropy_loss(scores_list[3], gt_classes).detach()
        else:
            loss_cls_mm = self.softmax_cross_entropy_loss(scores_list[0], gt_classes)
            loss_cls_v = self.softmax_cross_entropy_loss(scores_list[1], gt_classes)
            loss_cls_t = self.softmax_cross_entropy_loss(scores_list[2], gt_classes)
            # loss_cls_fuse = self.softmax_cross_entropy_loss(scores_list[3], gt_classes).detach()
        return {
            "loss_cls_mm": loss_cls_mm, 
            "loss_cls_v": loss_cls_v,
            "loss_cls_t": loss_cls_t,
            # "loss_cls_fuse": loss_cls_fuse,
    ...
    def inference(self, predictions, proposals):
        ...
        """
        predictions = (predictions[0], predictions[1], predictions[2])
    ...
    def image_label_losses(self, predictions, proposals, image_labels, \
        ...
        return {
            ...
            'loss_cls_t': score.new_zeros([1])[0],
            'loss_cls_v': score.new_zeros([1])[0],
            'loss_cls_mm': score.new_zeros([1])[0],
            'loss_box_reg': score.new_zeros([1])[0]}

    def image_label_losses_three(self, predictions, proposals, image_labels, \
        classifier_info=(None,None,None), ann_type='image'):
        '''
        Inputs:
            scores: N x (C + 1)
        ...
        scores_list = predictions[2]
        scores = scores.split(num_inst_per_image, dim=0) # B x n x (C + 1)
        scores_list = [scores_.split(num_inst_per_image, dim=0) for scores_ in scores_list]
        if self.with_softmax_prop:
            prop_scores = predictions[2].split(num_inst_per_image, dim=0)
        else:
            prop_scores = [None for _ in num_inst_per_image]
        ...
        storage = get_event_storage()
        loss = scores[0].new_zeros([1])[0]
        loss_v = scores[0].new_zeros([1])[0]
        loss_t = scores[0].new_zeros([1])[0]
        caption_loss = scores[0].new_zeros([1])[0]
        for idx, (score, score_v, score_t, labels, prop_score, p) in enumerate(zip(
            scores_list[0], scores_list[1], scores_list[2], image_labels, prop_scores, proposals)):
            if score.shape[0] == 0:
                loss += score.new_zeros([1])[0]
                continue
            if 'caption' in ann_type:
            ...
            for i_l, label in enumerate(labels):
                ...
                if self.image_label_loss in ['wsod', 'wsddn']: 
                    ...
                    loss_i, ind = self._max_size_loss(score, label, p)
                    loss_v_i, ind_v = self._max_size_loss(score_v, label, p)
                    loss_t_i, ind_t = self._max_size_loss(score_t, label, p)
                elif self.image_label_loss == 'first':
                    loss_i, ind = self._first_loss(score, label)
                elif self.image_label_loss == 'image':
                    loss_i, ind = self._image_loss(score, label)
                    ...
                    assert 0
                loss += loss_i / len(labels)
                loss_v += loss_v_i / len(labels)
                loss_t += loss_t_i / len(labels)
                if type(ind) == type([]):
                    img_box_count = sum(ind) / len(ind)
                    if self.debug:
                        for ind_i in ind:
        ...
        return {
            'image_loss_mm': loss * self.image_loss_weight,
            'image_loss_v': loss_v * self.image_loss_weight,
            'image_loss_t': loss_t * self.image_loss_weight,
            'loss_cls_t': score.new_zeros([1])[0],
            'loss_cls_v': score.new_zeros([1])[0],
            'loss_cls_mm': score.new_zeros([1])[0],
            'loss_box_reg': score.new_zeros([1])[0]}
    ...
    def forward(self, x, classifier_info=(None,None,None)):
        ...
        scores = []
        scores_mm, scores_v, scores_t, scores_fuse = [], [], [], []
        if classifier_info[0] is not None:
            logits_mm, logits_v, logits_t, logits_fuse = self.cls_score(x, classifier=classifier_info[0])
        else:
            logits_mm, logits_v, logits_t, logits_fuse = self.cls_score(x)
        scores_mm.append(logits_mm)
        scores_v.append(logits_v)
        scores_t.append(logits_t)
        scores_fuse.append(logits_fuse)
        if classifier_info[2] is not None:
            cap_cls = classifier_info[2]
            if self.sync_caption_batch:
                logits_mm, logits_v, logits_t, logits_fuse = self.cls_score(x, classifier=cap_cls[:, :-1]) 
            else:
                logits_mm, logits_v, logits_t, logits_fuse = self.cls_score(x, classifier=cap_cls)
            scores_mm.append(logits_mm)
            scores_v.append(logits_v)
            scores_t.append(logits_t)
            scores_fuse.append(logits_fuse)
        
        scores_mm = torch.cat(scores_mm, dim=1) # B x C' or B x N or B x (C'+N)
        scores_v = torch.cat(scores_v, dim=1) # B x C' or B x N or B x (C'+N)
        scores_t = torch.cat(scores_t, dim=1) # B x C' or B x N or B x (C'+N)
        scores_fuse = torch.cat(scores_fuse, dim=1)
        # print("score_mm shape: ",scores_mm.shape)
        proposal_deltas = self.bbox_pred(x)
        if self.with_softmax_prop:
            prop_score = self.prop_score(x)
            return scores_mm, proposal_deltas, prop_score, [scores_mm,scores_v,scores_t,scores_fuse]
        else:
            return scores_mm, proposal_deltas, [scores_mm,scores_v,scores_t,scores_fuse]
