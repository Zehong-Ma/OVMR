...
class MMDeticCascadeROIHeads(CascadeROIHeads):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.cls_score = MMZeroShotClassifier(cfg, input_shape)
                ...
                with storage.name_scope("stage{}".format(stage)):
                    if ann_type != 'box': 
                        ...
                        if ann_type in ['image', 'caption', 'captiontag']:
                            ...
                            weak_losses = predictor.image_label_losses_three(
                                (predictions[0], predictions[1], predictions[2]), proposals, image_labels,
                                classifier_info=classifier_info,
                                ann_type=ann_type)
                        ...
                        if self.with_image_labels:
                            stage_losses['image_loss_mm'] = \
                                predictions[0].new_zeros([1])[0]
                            stage_losses['image_loss_v'] = \
                                predictions[0].new_zeros([1])[0]
                            stage_losses['image_loss_t'] = \
                                predictions[0].new_zeros([1])[0]
    ...
    def _run_stage(self, features, proposals, stage, \
        ...
        if self.box_predictor[stage].cls_score is None:
            self.box_predictor[stage].cls_score = self.cls_score
