...
from .ins_zero_shot_classifier import InsZeroShotClassifier

__all__ = ["InsDeticFastRCNNOutputLayers"]


class InsDeticFastRCNNOutputLayers(FastRCNNOutputLayers):
    @configurable
    def __init__(
        self, 
    ...
    def from_config(cls, cfg, input_shape):
        ...
        if ret['use_zeroshot_cls']:
            ret['cls_score'] = None #InsZeroShotClassifier(cfg, input_shape)
        return ret
    ...
    def losses(self, predictions, proposals, \
        ...
        """
        scores, proposal_deltas, (rerank_logits, rerank_binary_labels, select_index, scores_rerank) = predictions
        gt_classes = (
        ...
        return {
            ...
            "loss_rerank_cls": loss_rerank_cls
        }
    ...
    def sigmoid_topk_cross_entropy_loss(self, pred_class_logits, target):
        if pred_class_logits.numel() == 0:
            return pred_class_logits.new_zeros([1])[0] # This is more robust than .sum() * 0.
        ...
        cls_loss = F.binary_cross_entropy_with_logits(
            pred_class_logits, target, reduction='mean') # B x topk
        # loss =  torch.sum(cls_loss) / B  
        return cls_loss
    ...
    def inference(self, predictions, proposals):
        ...
        """
        boxes = self.predict_boxes((predictions[0], predictions[1]), proposals)
    ...
    def forward(self, x, classifier_info=(None,None,None), labels=None):
        ...
        if self.with_softmax_prop:
            ...
            return cls_scores, proposal_deltas, prop_score, cls_scores_reranked
        else:
            return cls_scores, proposal_deltas, cls_scores_reranked
