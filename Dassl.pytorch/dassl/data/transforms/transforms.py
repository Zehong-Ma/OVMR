...
class ResizeWithPad:
    """Resize with bidirectional padding
    """
...
def _build_transform_train(cfg, choices, target_size, normalize):
    ...
    if all(conditions):
        ...
        print(f"+ resize smaller edge to {target_size}")
...
def _build_transform_eval(cfg, choices, target_size, normalize):
    print("Building transform_train")
    tfm_train = []
    ...
    if "random_resized_crop" in choices:
        s_ = cfg.INPUT.EVAL_RRCROP_SCALE
        print(f"+ random resized crop (size={input_size}, scale={s_})")
        tfm_train += [
            RandomResizedCrop(input_size, scale=s_, interpolation=interp_mode)
        ]
    ...
    if "colorjitter" in choices:
        b_ = cfg.INPUT.COLORJITTER_B/2
        c_ = cfg.INPUT.COLORJITTER_C/2
        s_ = cfg.INPUT.COLORJITTER_S/2
        h_ = cfg.INPUT.COLORJITTER_H/2
        print(
            f"+ color jitter (brightness={b_}, "
            f"contrast={c_}, saturation={s_}, hue={h_})"
        )
        tfm_train += [ RandomApply(
            [ColorJitter(
                brightness=b_,
                contrast=c_,
                saturation=s_,
...
def _build_transform_test(cfg, choices, target_size, normalize):
    ...
    # print(f"+ {target_size} resize with pad")
    # tfm_test += [ResizeWithPad(input_size[0], input_size[1])]
