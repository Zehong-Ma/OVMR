...
class RandomClassSampler(Sampler):
    ...
    def __iter__(self):
        ...
        while len(avai_labels) >= self.ncls_per_batch or len(avai_labels)>0: # dont drop last
            if len(avai_labels) < self.ncls_per_batch:
                selected_labels = random.sample(avai_labels, len(avai_labels))
            else:
...
class RandomFullClassSampler(Sampler):
    """Randomly samples N classes with all shots
    form a minibatch of size N*K.
    ...
    def __iter__(self):
        ...
        while len(avai_labels)>0: # dont drop last
            if len(avai_labels) < self.ncls_per_batch:
                selected_labels = random.sample(avai_labels, len(avai_labels))
            else:
                selected_labels = random.sample(avai_labels, self.ncls_per_batch)
            for label in selected_labels:
                batch_idxs = batch_idxs_dict[label]
                final_idxs.extend(batch_idxs)
                avai_labels.remove(label)
...
def build_sampler(
    ...
    elif sampler_type == "RandomFullClassSampler":
        return RandomFullClassSampler(data_source, batch_size, n_ins)
